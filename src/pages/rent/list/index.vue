<template>
  <div class="pageContent bg_fff" :class="(showMoreShop || showMoreEquip)?'over_hidden':''">
    ...
    <div class="filterMenu pt15">
      <!-- 一级分类 -->
      <ul
        class="tabList menu flex"
        :class="{'mr-55':oneTypeList.length<3,'justifyContentBetween':oneTypeList.length>2}"
        v-if="oneTypeList.length>0"
      >
        <li
    ...
    <div v-for="(filterItem,filterIndex) in filterMenu" :key="filterIndex">
      ...
      >
        <div class="scroll flex1">
          <div
            @click="selectAreaTab(1,{Child:[]},filterItem,-1)"
            :class="{'active':filterItem.selectIndex===-1}"
          >
            <p>不限</p>
          </div>
          <div
        ...
        <div
          class="scroll flex1"
          v-if="areaTwoList.length>0"
          style="border-left:1rpx solid #f2f2f2"
        >
          <div
            @click="selectAreaTab(2,{Name:''},filterItem,-1)"
      ...
      <!-- 更多的弹窗 -->
      <div class="modal_mask more__modal_mask" v-if="isShadeType == 'More' && filterItem.selected ">
        <div class="modal__bd">
...
export default {
  ...
  methods: {
    init() {
      this.cityCode = this.CityCode || location(this);

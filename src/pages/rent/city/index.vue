<template>
  <div class="pageCity">
      <!--顶部输入框-->
      <div class="bg_fff">
          <div class="topinput flex flexAlignCenter">
              <img src="/static/images/icons/search.png" class="icon_search absolu">
              <input type="text" placeholder="请输入城市名称" :value="inputName" @input="bindKeyInput" class="flex1">
              <img src="/static/images/icons/cancle.png" class="icon-del absolu" style="z-index:40" @click="bindBlur">
          </div>
      </div>
      <!--右侧字母 左侧列表-->
      <div class="container-inner">
          ...
          <div class="container">
              <scroll-view scroll-y="true" v-bind:style="{height: winHeight + 'px'}" :scroll-into-view="scrollTopId">
                ...
                <div class="flex cityhot justifyContentAround">
                    <div class="name" v-for="item in hotCity" :key="item.id" :data-city="item.name"  @click="bindCity(item.id)">{{item.name}}</div>
                </div>
                <!--搜索城市-->
                ...
                <!--城市列表-->
                  ...
                  <div class="item titleClomn"> 其他区域敬请期待！</div>
              </scroll-view>
          </div>
      </div>
